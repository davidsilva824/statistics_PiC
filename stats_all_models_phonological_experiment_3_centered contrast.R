...
setwd("C:/Users/Admin/Desktop/Dissertação/código/satistics_PiC/Statistics_PiC/results_phonological_compounds/experiment_3/all_results")

# Get list of all CSV files for THIS experiment in the folder
file_list <- list.files(pattern = "^results_experiment_3_.*\\.csv$", ignore.case = TRUE)

# currently it is reading all the files in the folder. But alternatively you can add a list of files:
#file_list <- c("results_experiment_2_babble_phonetic_BPE_with_spaces.csv", ...)
...
for (current_file in file_list) {
  ...
  # 2) plurality = derived from Category
  # 3) sibilance = derived from Category
  
  dat <- dat %>%
    mutate(
      set = paste0("set_", ((row_number() - 1) %/% 4) + 1),
      plurality = ifelse(grepl("Plural", Category), "Plural", "Singular"),
      sibilance = ifelse(grepl("Sibilant", Category), "Sibilant", "NonSibilant"),
...
if (length(winners_list) > 0) {
  ...
  write.csv(final_table, "winning_models_results.csv", row.names = FALSE)
  
  print(final_table, right = FALSE, row.names = FALSE)
} else {
  cat("No converged models were found.\n")
}