...
#"results_experiment_1_phonemetransformers__GPT2-85M-BPE-TXT-SPACELESS.csv",
...
for (current_file in file_list) {
  ...
  dat <- dat %>% 
    mutate(
      ...
      plurality  = factor(plurality,  levels = c("Plural", "Singular"))   # changed to Plural first
    )
...
#### Testing with the plural as reference. 
...
isSingular(final_model, tol = 1e-6)

# -----------------------------------------------------------------------------------------------------------

#### Testing with the singular as reference. 
...
# Keep plurality_num consistent with the reference (0 = ref, 1 = other)
dat_sing$regularity_num <- ifelse(dat_sing$regularity == "Regular", 0, 1)
dat_sing$plurality_num  <- ifelse(dat_sing$plurality  == "Singular", 0, 1)

# keep your same 'set' mapping
dat_sing <- dat_sing %>%
  mutate(
    set = case_when(
      Non.Head %in% c("goose","geese","swan","swans") ~ "set_goose_swan",
...
summary(final_model_singref)
isSingular(final_model_singref)
