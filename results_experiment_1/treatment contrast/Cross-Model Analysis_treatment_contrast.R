...
selected_formula <- winner$formula[1]

# -------------------------------------------------------------------------------------------
### Run selected model with normal references

cat("\n--- SELECTED MODEL: Normal references ---\n\n")

warning_messages <- character(0)

m_normal <- tryCatch(
  withCallingHandlers(
    lmer(as.formula(selected_formula), dat, REML = TRUE),
    warning = function(w){
...
# -------------------------------------------------------------------------------------------
### Run selected model with inverted references

cat("\n--- SELECTED MODEL: Inverted references ---\n\n")
...
if (inherits(m_inverted, "error")) {
  cat("ERROR:", conditionMessage(m_inverted), "\n")
  if(length(warning_messages) > 0){
    cat("Warnings:\n")
    for(w in unique(warning_messages)) cat("-", w, "\n")
  }
  ...
  s_inverted <- withCallingHandlers(
    summary(m_inverted),
    warning = function(w){
      warning_messages <<- c(warning_messages, conditionMessage(w))
      invokeRestart("muffleWarning")
  ...
  cat("AIC:", as.numeric(AIC(m_inverted)),
      " | convergence:", hasConverged(m_inverted),
      " | singular:", isSingular(m_inverted), "\n")
  cat("Formula used:", paste(deparse(formula(m_inverted), width.cutoff = 500), collapse = " "), "\n")
  ...
  cat("\n")
  print(round(as.data.frame(s_inverted$coefficients), 4))
  cat("\n")
}
