...
top_10 <- all_results[all_results$convergence == 1, ]

if(nrow(top_10) == 0) stop("No models converged in the global fit.")

# order by AIC and keep first 10
top_10 <- top_10[order(top_10$AIC), ]
top_10 <- head(top_10, 10)

cat("\n--- TOP 10 MODELS BY AIC ---\n\n")
print(top_10[, c("AIC","convergence","formula","family")], row.names = FALSE)

write.csv(
  top_10[, c("AIC","convergence","formula","family")],
  "top_10_models_by_AIC.csv",
  row.names = FALSE
)
