...
# To read only some  files in a folder:
#file_list <- c(
...
out_file <- "global_model_selection_centered_results.txt"
...
df_g1 <- fit_formula_list(fr_g1, dat, "model_|__set_|__Head_|")
...
df_g2 <- fit_formula_list(fr_g2, dat, "model_||__set_||__Head_||")
...
df_g3 <- fit_formula_list(fr_g3, dat, "model_|__set_||__Head_||")
...
df_g4 <- fit_formula_list(fr_g4, dat, "model_||__set_|__Head_|")
...
selected_formula_normal <- winner$formula[1]
selected_formula_nested <- sub("regularity \\* plurality", "regularity / plurality", selected_formula_normal)

# -------------------------------------------------------------------------------------------
### Run selected model: normal

cat("\n--- SELECTED MODEL: Centered normal ---\n\n")

warning_messages <- character(0)

m_normal <- tryCatch(
  withCallingHandlers(
    lmer(as.formula(selected_formula_normal), dat, REML = TRUE),
    warning = function(w){
...
if (inherits(m_normal, "error")) {
  cat("ERROR:", conditionMessage(m_normal), "\n")
  if(length(warning_messages) > 0){
    cat("Warnings:\n")
    for(w in unique(warning_messages)) cat("-", w, "\n")
  }
  cat("\n")
} else {
  s_normal <- withCallingHandlers(
    summary(m_normal),
    warning = function(w){
      warning_messages <<- c(warning_messages, conditionMessage(w))
      invokeRestart("muffleWarning")
  ...
  cat("AIC:", as.numeric(AIC(m_normal)),
      " | convergence:", hasConverged(m_normal),
      " | singular:", isSingular(m_normal), "\n")
  cat("Formula used:", paste(deparse(formula(m_normal), width.cutoff = 500), collapse = " "), "\n")
  ...
  cat("\n")
  print(round(as.data.frame(s_normal$coefficients), 4))
  cat("\n")
}

# -------------------------------------------------------------------------------------------
### Run selected model: nested

cat("\n--- SELECTED MODEL: Centered nested ---\n\n")

warning_messages <- character(0)

m_nested <- tryCatch(
  withCallingHandlers(
    lmer(as.formula(selected_formula_nested), dat, REML = TRUE),
    warning = function(w){
      warning_messages <<- c(warning_messages, conditionMessage(w))
      invokeRestart("muffleWarning")
    }
...
if (inherits(m_nested, "error")) {
  cat("ERROR:", conditionMessage(m_nested), "\n")
  if(length(warning_messages) > 0){
    cat("Warnings:\n")
    for(w in unique(warning_messages)) cat("-", w, "\n")
  }
  ...
  cat("AIC:", as.numeric(AIC(m_nested)),
      " | convergence:", hasConverged(m_nested),
      " | singular:", isSingular(m_nested), "\n")
  cat("Formula used:", paste(deparse(formula(m_nested), width.cutoff = 500), collapse = " "), "\n")
  ...
  cat("\n")
  print(round(as.data.frame(s_nested$coefficients), 4))
  cat("\n")
}
