...
# Load data
dat <- read.csv("results_LGT_BERT_100M_experiment_3.csv", check.names = TRUE)
...
final_model_1 <- lmer(
  Surprisal.head ~ 1 + regularity * plurality + (1 | set) + (1 | Head), 
  data = dat,
  REML = TRUE 
)
...
final_model_2 <- lmer(
  Surprisal.head ~ 1 + regularity * plurality + (1 + plurality | set) + (1 | Head), 
  data = dat,
  REML = TRUE 
)
