...
# Load data
dat <- read.csv("results_babyLlama_100M_experiment_3.csv", check.names = TRUE)
...
### notas da reunião 
contrasts(df$regularity) <- c(-0.5, 0.5)
contrasts(df$plurality) <- c(-0.5, 0.5)

df$regularity_num <- ifelse(df$regularity == "Regular", -0.5, 0.5)
df$plurality_num <- ifelse(df$plurality== "Singular", -0.5, 0.5)




df$regularity_num <- ifelse(df$regularity == "Regular", -0.5, 0.5) # meter a bater certo com os contraste 
...
final_model_2 <- lmer(
  Surprisal.head ~ 1 + regularity * plurality + (1 + regularity + plurality | set) + (1 + regularity + plurality | Head), 
  data = dat,
  REML = TRUE 
)
