...
setwd("C:/Users/Admin/Desktop/Dissertação/código/satistics_PiC/Statistics_PiC/results_berent&pinker/experiment_2_with_stories/10M")
...
m <- brm(Surprisal.head ~ 1 + type * plurality +
         ...
         cores = 4,
         backend = "cmdstanr",
         file = "C:/Users/Admin/Desktop/Dissertação/código/satistics_PiC/Statistics_PiC/Bayesean_Statistics/surprisal_gaussian_sum_experiment_2_10M_with_sentences")
...
m_cmat <- unique(m_mm)
rownames(m_cmat) <- paste(levels(dat$type) |> rep(each = 2), levels(dat$plurality))
m_preds <- t(m_cmat %*% t(m_samples))

m_cmat
posterior_summary(m_preds)

(hyp_reg <- hypothesis(m, "plurality1 + 0.5 * type1:plurality1 = 0"))
log(1 / hyp_reg$hypothesis$Evid.Ratio)

(hyp_sib <- hypothesis(m, "plurality1 - 0.5 * type1:plurality1 = 0"))
log(1 / hyp_sib$hypothesis$Evid.Ratio)

(hyp_int <- hypothesis(m, "type1:plurality1 = 0"))
log(1 / hyp_int$hypothesis$Evid.Ratio)

(m_eff_reg <- hypothesis(m, "plurality1 + 0.5 * type1:plurality1 > 0",
                         scope = "coef", group = "model", alpha = 0.025)$hypothesis)
(m_eff_sib <- hypothesis(m, "plurality1 - 0.5 * type1:plurality1 > 0",
                         scope = "coef", group = "model", alpha = 0.025)$hypothesis)
(m_eff_int <- hypothesis(m, "type1:plurality1 > 0",
                         scope = "coef", group = "model", alpha = 0.025)$hypothesis)

m_eff_int$panel <- "Interaction"
m_eff_reg$panel <- "Regular effect"
m_eff_sib$panel <- "Sibilant effect"
m_eff_all <- rbind(m_eff_int, m_eff_reg, m_eff_sib)

m_eff_all$panel <- factor(m_eff_all$panel, levels = c("Regular effect", "Sibilant effect", "Interaction"))
